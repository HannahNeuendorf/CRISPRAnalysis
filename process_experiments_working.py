# -*- coding: utf-8 -*-
"""
Created on Thu Nov 17 15:04:04 2022
...
def processExperimentsFromConfig(configFile, libraryDirectory, generatePlots='png'):
    ...
    for (phenotype, condition1, condition2) in exptParameters['condition_tuples']:
        for replicate in replicateList:
            print(f"condition1: {condition1} replicate: {replicate}")
    ...
    if len(exptParameters['analyses']) > 0:
        ...
        geneTable.fillna(0, inplace=True)
        ...
        #### Calculate best average for each comparison based on geneTableCollapsed
        best_avg_l = []
        
        # Iterate over each unique experiment
        for expt in geneTableCollapsed.columns.get_level_values(0).unique():
            print(f"Experiment: {expt}")
            
            geneTableCollapsed_expt = geneTableCollapsed[expt]

            # Get list of unique replicates
            rep_l = geneTableCollapsed_expt.columns.get_level_values(0).unique()

            # Get list of replicates without the ave_ column
            no_ave_rep_l = [i for i in rep_l if i[:4] != "ave_"]

            # Sum over no_avg_rep_l columns and divide by number of replicates to get best average
            best_sum = geneTableCollapsed_expt[no_ave_rep_l].groupby(level=1, axis=1).agg(sum)
            numerics = [
                "int",
                "int16",
                "int32",
            ...
            best_avg = best_sum.select_dtypes(include=numerics) / len(no_ave_rep_l)
            # best_avg = np.divide(best_sum, len(no_ave_rep_l), where=best_sum.dtypes.ne(object))

            # Clone non-numerical colums of best_sum to best_avg
            non_num_cols = best_sum.select_dtypes(exclude=numerics).columns.tolist()
            best_avg[non_num_cols] = best_sum[non_num_cols]

            # Add experiment and replicate level to multi-index column of best_avg
            best_avg.columns = pd.MultiIndex.from_product(
                [[expt], [f"best_avg_{'_'.join(no_ave_rep_l)}"], best_avg.columns],
            )

            # Append to best_avg_l to concatenate back to geneTableCollapsed later
            best_avg_l.append(best_avg)

        # Concate best_avg together, and then concatenate them back into geneTableCollapse
        # Also sort columns so experiments go together
        geneTableCollapsedUpdated = pd.concat(
            [pd.concat(best_avg_l, axis=1), geneTableCollapsed], axis=1
        ).sort_index(axis=1)
        geneTableCollapsedUpdated.to_csv(outbase + '_genetable_collapsed_updated.txt',sep='\t')
...
def scoreGeneByBestTranscript(geneTable):
    ...
    for tup, group in geneTable.groupby(level=list(range(2)),axis=1):
        tupList.append(tup)
        print(f"bestTranscriptFrame.index: {bestTranscriptFrame.index} bestTranscriptFrame[tup]: {bestTranscriptFrame[tup]}", flush=True)
...
def applyMW(group, negativeTable):
    if tuple(map(int, sp.__version__.split('.')))[0:2] >= (0, 17): #implementation of the "alternative flag":
